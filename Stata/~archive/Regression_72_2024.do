********************************************************************************
* Impact of NEA Codeshare with Market Fixed Effects 7/6/2024, Zheyu
********************************************************************************
...
/*
preserve 
	collapse (sum)passengers , by(market year quarter)
	hist passengers if passengers>10000, frequency
restore 
*/
...
reg mktfare roundtrip nonstop t_codeshare v_codeshare  pure_online ///
...
reg lnmktfare roundtrip nonstop t_codeshare v_codeshare  pure_online ///
 1.b6aa_c#1.AA 1.b6aa_c#1.B6 WN AA DL UA NK AS B6 F9 G4 HA SY XP MX MM  ///
 nea_market_codeshared i.dt i.mktgroupid [aweight = total_quantity], cluster(market)

est sto reg_mf_airportsgroupln

reg MktFare_sd roundtrip nonstop t_codeshare v_codeshare pure_online ///
 1.b6aa_c#1.AA 1.b6aa_c#1.B6 WN AA DL UA NK AS B6 F9 G4 HA SY XP MX MM  ///
 nea_market_codeshared i.dt i.mktgroupid [aweight = passengers], cluster(market)
...
*display _b[1.nea_market_codeshared#1.JetBlue_new_market_served]
...
using result722024mf_jbn.xls, cells("b(star fmt(4))" se)stats(r2 N)  replace
...
reg_mf_airportsgroupssln reg_mf_airportsgroupsssd using result722024mf.xls, cells("b(star fmt(4))" se)stats(r2 N) replace
...
foreach var of local dependent_vars {
    * Run the regression model
    quietly reg ln`var' roundtrip nonstop t_codeshare v_codeshare pure_online ///
 1.b6aa_c#1.AA 1.b6aa_c#1.B6 WN AA DL UA NK AS B6 F9 G4 HA SY XP MX MM  ///
 nea_market_codeshared 1.nea_market_codeshared#1.dummy_bos 1.nea_market_codeshared#1.dummy_jfk ///
1.nea_market_codeshared#1.dummy_ewr 1.nea_market_codeshared#1.dummy_lga i.dt i.mktgroupid [aweight = passengers], cluster(market)
...
recast(connected)  ciopts(recast(rarea) color(gs14) lpattern(dash)) ///
graphregion(color(white)) plotregion(color(white))
graph export "E:\Research\Codeshare JetBlue AA\Stata File\Price_Dispersion_MFE_EWR_carriergroup.png", replace
...
recast(connected)  ciopts(recast(rarea) color(gs14) lpattern(dash)) ///
graphregion(color(white)) plotregion(color(white))
graph export "E:\Research\Codeshare JetBlue AA\Stata File\Price_Dispersion_MFE_Other_carriergroup.png", replace
...
reg passengers roundtrip nonstop t_codeshare v_codeshare pure_online ///
 1.b6aa_c#1.AA 1.b6aa_c#1.B6 WN AA DL UA NK AS B6 F9 G4 HA SY XP MX MM  ///
 nea_market_codeshared 1.nea_market_codeshared#1.JetBlue_new_market_served ///
...
estout reg_mf_quantity reg_mf_quantity_airport using result722024mfquantity.xls, cells("b" se)  replace
